...
static int set_mqtt_connect_params(TemplateInitParams *initParams)
{
    int ret = HAL_GetDevInfo((void *)&sg_devInfo);
    if (QCLOUD_RET_SUCCESS != ret) {
        ESP_LOGE(TAG, "get devinfo fail");
        return ret;
    }
...
static int smart_light_template_init()
{
    memset((void *)&sg_ProductData, 0, sizeof(ProductDataDefine));
}
...
int smart_light_demo_test(void)
{
    ...
    // init connection  keep-alive times : 240s
    TemplateInitParams init_params = DEFAULT_TEMPLATE_INIT_PARAMS;
    rc = set_mqtt_connect_params(&init_params);
    if (rc != QCLOUD_RET_SUCCESS) {
        ESP_LOGE(TAG, "init params err,rc=%d", rc);
        return rc;
    }
