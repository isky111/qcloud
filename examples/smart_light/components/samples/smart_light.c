...
int smart_light_demo(void)
{
    ...
    while (IOT_Template_IsConnected(client) || rc == QCLOUD_ERR_MQTT_ATTEMPTING_RECONNECT ||
        ...
        if (sg_control_msg_arrived) {
            ...
            if (rc == QCLOUD_RET_SUCCESS) {
                ESP_LOGI(TAG, "Contol msg reply success");
                sg_control_msg_arrived = false;
            }
            else {
                ESP_LOGE(TAG, "Contol msg reply failed, err: %d", rc);
            }
        }
        ...
        if (QCLOUD_RET_SUCCESS == deal_up_stream_user_logic(pReportDataList, &ReportCount))
        {
            ...
            if (rc == QCLOUD_RET_SUCCESS) {
                ...
                if (rc == QCLOUD_RET_SUCCESS) {
                    ESP_LOGI(TAG, "data template report success");
                }
                else {
                    ESP_LOGE(TAG, "data template report failed, err: %d", rc);
                }
            }
            else{
                ESP_LOGE(TAG, "construct report data failed, err: %d", rc);
            }
        }
